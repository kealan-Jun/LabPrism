...
def run(request_path,output):
 ...
 torch.set_num_threads(4);cv2.setNumThreads(2);model=JointSegmenter(m['path'],m['sha256'],m['role'],parent['source']['camera_role']);output.mkdir(parents=True,exist_ok=False);evidence=output/'evidence';evidence.mkdir()
