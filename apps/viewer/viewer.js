...
async function loadClip(item){
  ...
  for(const id of ["trails","semantic"]){$(id).checked=false;$(id).disabled=true;}
  $("semantic-note").textContent="";$("conflicts").textContent="";
  ...
  try{
    ...
    if(!["labprism-video-result/1","labprism-video-result/2"].includes(result.schema_version)||!result.frames.length)throw Error("结果契约不支持或没有分析帧");
    ...
    const candidate=data.schema_version==="labprism-video-result/2";
    ...
    if(candidate)entries.push(["候选状态","轨迹身份待验证；语义域外误分明显；类别冲突保留。未晋级质量验收。"]);
...
for(const id of ["boxes","masks","hands","labels","original","trails","semantic"])$(id).addEventListener("change",()=>render(true));
